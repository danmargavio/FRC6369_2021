...
public class DrivetrainSubsystem extends Subsystem {
    public static final double TRACKWIDTH = 20.5;
    public static final double WHEELBASE = 25.5;

    public static double floff = 284.0;
    public static double froff = 73.5;
    public static double bloff = 118.2;
    public static double broff = 238.9;

    private static double FRONT_LEFT_ANGLE_OFFSET = -Math.toRadians(133.0);//metal on wheels facing starbert
    private static double FRONT_RIGHT_ANGLE_OFFSET = -Math.toRadians(95.3);
    private static double BACK_LEFT_ANGLE_OFFSET = -Math.toRadians(120.7);
    private static double BACK_RIGHT_ANGLE_OFFSET = -Math.toRadians(255.1);

    public void zeroDriveTrain(){
        floff = Math.toDegrees(frontLeftModule.getCurrentAngle()) - floff;
        froff = Math.toDegrees(frontRightModule.getCurrentAngle()) - froff;
        bloff = Math.toDegrees(backLeftModule.getCurrentAngle()) - bloff;
        broff = Math.toDegrees(backRightModule.getCurrentAngle()) - broff;
    }
    ...
    public void periodic() {
        ...
        SmartDashboard.putNumber("gyroscope angle", getAngle(imu.getAngle()));
        SmartDashboard.putNumber("Xvelocity", imu.getVelocityX());
        SmartDashboard.putNumber("Yvelocity", imu.getVelocityY());
    ...
    public void drive(Translation2d translation, double rotation, boolean fieldOriented) {
        ...
        if (fieldOriented) {
            //speeds = ChassisSpeeds.fromFieldRelativeSpeeds(translation.getX(), translation.getY(), rotation, Rotation2d.fromDegrees(gyroscope.getAngle().toDegrees())); //Rotation2d.fromDegrees(0.0));
            speeds = ChassisSpeeds.fromFieldRelativeSpeeds(translation.getX(), translation.getY(), rotation, Rotation2d.fromDegrees(getAngle(imu.getAngle()))); //Rotation2d.fromDegrees(0.0));
        } else {
